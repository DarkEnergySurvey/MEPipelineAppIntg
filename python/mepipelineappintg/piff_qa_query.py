...
def get_piff_qa(fname,piff_tag,mAttID,dbh,dbSchema,verbose=0):
    ...
    if qa_data is False:
        raise ValueError("ERROR: Query to PIFF_MODEL_QA (tag={ptag:s}) returned no entries corrseponding to {tname:s} (tag={mtag:s}).".format(
            ptag=piff_tag,tname=tilename,mtag=me_tag))
    else:
        print("# Sucessfull query for PIFF_MODEL_QA")
...
#    print(qa_data)
    ...
    print("# Wrote PIFF_MODEL_QA to: {ftab:s}".format(ftab=fname))
